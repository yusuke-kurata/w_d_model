...
def build_model_columns():
  ...
  #deepモデルの特徴量の設定　
  deep_columns = [
      tf.feature_column.indicator_column(classVal),      tf.feature_column.indicator_column(holiday_flag),
...
def input_fn(data_file, num_epochs, shuffle, batch_size):
  ...
  def parse_csv(value):
    ...
    # key:_CSV_COLUMNS, value:Tensor型のcolumns の辞書を作成
    features = dict(zip(_CSV_COLUMNS, columns))

    # labels = features.pop('holiday_flag')
    labels = features.pop('item_id')
    # names_tf = ops.convert_to_tensor(labels, dtype=dtypes.string)
    # names_str = '1'
